...
def not_short_check(drives):
    ...
    if len(drives) == 1:
        ...
        vid_info = next((stream for stream in probe['streams'] if stream['codec_type'] == 'video'), None)
...
def trim_files(drives, outputfolder):
    ...
    for i, (f, _) in enumerate(zip(drives, drives[1:])):
        ...
        ffmpeg.input(f, to=duration-1).output(outname, c="copy").run(overwrite_output=True)
        files[i] = outname
...
def concatenate(files, output):
    ...
    # concatenate demuxer
    ffmpeg.input(videolist, format="concat", safe=0).output(output, c="copy").run(overwrite_output=True)
